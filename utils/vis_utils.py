...
def run_inference_and_visualize(model, dataset, device, output_dir, num_samples=5):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if predicted_masks.ndim == 2:
            predicted_masks = np.repeat(predicted_masks[np.newaxis, :, :], num_categories, axis=0)
        ...
        if ground_truth_masks.shape[-2:] != predicted_masks.shape[-2:]:
            ground_truth_masks = interpolate(inputs["ground_truth_masks"].float(),
        ...
        for j, (gt_mask, pred_mask) in enumerate(zip(ground_truth_masks, predicted_masks)):
